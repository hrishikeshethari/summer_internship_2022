...
class Task2:
    ...
    def __init__(self):
        ...
        self.test_data1 = dict()
        self.test_data2 = dict()
        self.g1 = nx.Graph()
        self.g2 = nx.Graph()
        self.s = list()
        self.s1 = list()
    ...
    def build_files_source(self):
        ...
        for file in self.file_path_data.keys():
            ...
            if pos != -1:
                ...
                if "src" in modules:
                    print(c)
                    # print(f"file = {file} | path = {self.file_path_data[file]}")
                    if file not in self.file_source:
                        self.file_source.append(file)
        print(f"Number of source code files = {len(self.file_source)}")

    def build_commit_set_of_files(self, project):
        for element in self.commit_file_records:
            commit_id = element["commit_id"]
            if self.commit_project_data[commit_id] == project:
                file_id = element["file_id"]
                if file_id in self.file_source:
                    # print(f"file in source = {file_id}")
                    if commit_id not in self.commit_set_of_files.keys():
                        self.commit_set_of_files[commit_id] = list()
                    if file_id not in self.commit_set_of_files[commit_id]:
                        self.commit_set_of_files[commit_id].append(file_id)
                    # print(f"commit = {commit_id} | files = {self.commit_set_of_files[commit_id]}")
            '''if commit_id not in self.commit_set_of_files.keys():
            ...
            for commit in self.commit_set_of_files:
                c = c + 1
                print(f"{c}.commit = {commit} | files = {self.commit_set_of_files[commit]}")'''
        print(f"Length of commit set of files = {len(self.commit_set_of_files)}")
    ...
    def print_data(self, project):
        ...
        for commit in self.commit_time:
            ct = ct + 1
            if self.commit_project_data[commit] == project:
                temp = self.commit_time[commit] - t2
                temp_days, temp_seconds = temp.days, temp.seconds
                temp_hours = temp_days * 24 + temp_seconds // 3600
                if ct == 1:
                    m = temp_hours - td_hour
                if m >= temp_hours - td_hour >= 0:
                    t3 = self.commit_time[commit]
                    m = temp_hours - td_hour
                    m1 = t3 - t2
                    m1_days, m1_seconds = m1.days, m1.seconds
                    m1_hours = m1_days * 24 + m1_seconds // 3600
                    if m1_hours > 24:
        ...
        for commit in self.commit_time:
            ct = ct + 1
            if self.commit_project_data[commit] == project:
                temp = self.commit_time[commit] - t3
                temp_days, temp_seconds = temp.days, temp.seconds
                temp_hours = temp_days * 24 + temp_seconds // 3600
                if ct == 1:
                    m = temp_hours - td_3hours
                if m >= temp_hours - td_3hours >= 0:
        ...
        for commit in self.commit_time:
            ct = ct + 1
            if self.commit_project_data[commit] == project:
                temp = self.commit_time[commit] - t4
                temp_days, temp_seconds = temp.days, temp.seconds
                temp_hours = temp_days * 24 + temp_seconds // 3600
                if ct == 1:
                    m = temp_hours - td_4hours
                if m >= temp_hours - td_4hours >= 0:
        ...
        for commit in self.commit_time:
            ct = ct + 1
            if self.commit_project_data[commit] == project:
                temp = self.commit_time[commit] - t5
                temp_days, temp_seconds = temp.days, temp.seconds
                temp_hours = temp_days * 24 + temp_seconds // 3600
                if ct == 1:
                    m = temp_hours - td_hour
        ...
        for commit in self.commit_time:
            ct = ct + 1
            if self.commit_project_data[commit] == project:
                temp = self.commit_time[commit] - t6
                temp_days, temp_seconds = temp.days, temp.seconds
                temp_hours = temp_days * 24 + temp_seconds // 3600
                if ct == 1:
                    m = temp_hours - td_3hours
                if m >= temp_hours - td_3hours >= 0:
                    t7 = self.commit_time[commit]
                    m = temp_hours - td_3hours'''
        t6 = t7 - relativedelta(months=3)
        t5 = t6 - timedelta(days=1)
        t4 = t5 - relativedelta(months=4)
        t3 = t4 - relativedelta(months=3)
        t2 = t3 - timedelta(days=1)
        t1 = t2 - relativedelta(months=7)
        print(f"t1 = {t1}")
        print(f"t2 = {t2}")
        print(f"t3 = {t3}")
        print(f"t4 = {t4}")
        ...
        self.build_test_data(t3, t5, t6, t7, project)

    def build_test_data(self, t3, t5, t6, t7, project):
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t3 <= self.commit_time[commit] <= t5:
                    self.test_data1[commit] = self.commit_time[commit]
                if t6 <= self.commit_time[commit] <= t7:
                    self.test_data2[commit] = self.commit_time[commit]
        # print(f"Training data = {self.training_data}")
        # print("\n\n\n\n\n")
        # print(f"Test data = {self.test_data}")

    def build_test_file1(self, project):
        for commit in self.commit_set_of_files:
            if self.commit_project_data[commit] == project:
                if commit in self.test_data1:
        ...
        print(f"Length of edges test data 1 = {len(self.g1.edges())}")
        print(f"Length of nodes test data 1 = {len(self.g1.nodes())}")

    def build_test_file2(self, project):
        for commit in self.commit_set_of_files:
            if self.commit_project_data[commit] == project:
                if commit in self.test_data2:
                    files = self.commit_set_of_files[commit]
                    if len(files) > 1:
                        ordered_pairs = {(x, y) for x in files for y in files if x != y}
                        self.g2.add_edges_from(ordered_pairs)
        # print(self.g2.degree)
        print(f"Length of edges test data 2 = {len(self.g2.edges())}")
        print(f"Length of nodes test data 2 = {len(self.g2.nodes())}")
    ...
    def build_link_prediction(self):
        ...
        result_file = open("F:\\task2b_test_imbalance_6time_str2_storm_source.csv", "w")
...
if __name__ == "__main__":
    ...
    t.build_file_path()
    print("\nSource code files")
    t.build_files_source()
    print("\nCommit set of files")
    t.build_commit_set_of_files("storm")
    ...
    # t.print_data("kafka")
    # t.build_test_file1("kafka")
    # t.build_test_file2("kafka")
    t.print_data("storm")
    t.build_test_file1("storm")
    t.build_test_file2("storm")
    t.compare_graph()
