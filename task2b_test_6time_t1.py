...
class Task2:
    ...
    def print_data(self, project):
        ...
        t1 = min(time_list)
        t6 = max(time_list)
        print(f"Min time t1m = {t1}")
        print(f"Max time t6 = {t6}")
        td = t6 - t1
        print(f"td = {td}")
        td_days, td_seconds = td.days, td.seconds
        td_hours = td_days * 24 + td_seconds // 3600
        N = td_hours
        print(f"N = {N}")
        t2d_hours = (N - 2) / 3
        print(f"t2dh from t1 = {t2d_hours}")
        ct = 0
        t2 = 0
        m = 1000
        for commit in self.commit_time:
        ...
        print(f"t2 = {t2}")
        m1 = t2.month + 1
        t3 = t2.replace(month=m1)
        print(f"t3 = {t3}")
        t4d_hours = (N - 2) / 3
        print(f"t4dh from t3 = {t4d_hours}")
        ct = 0
        t4 = 0
        m = 1000
        for commit in self.commit_time:
            ...
            if self.commit_project_data[commit] == project:
                ...
                # temp_seconds = temp_seconds % 60
                if ct == 1:
                    m = temp_hours - t4d_hours
                if m >= temp_hours - t4d_hours >= 0:
                    t4 = self.commit_time[commit]
                    m = temp_hours - t4d_hours
        print(f"t4 = {t4}")
        m2 = t4.month + 1
        t5 = t4.replace(month=m2)
        print(f"t5 = {t5}")
        t6d_hours = (N - 2) / 3
        print(f"t6dh from t5 = {t6d_hours}")
        t6d = t6 - t5
        print(f"t6d = {t6d}")
        t6d_days, t6d_seconds = t6d.days, t6d.seconds
        t6dh = t6d_days * 24 + t6d_seconds // 3600
        print(f"t6dh = {t6dh}")
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t1 <= self.commit_time[commit] <= t6:
                    c = c + 1
        print(f"Number of commits between t1 and t6 = {c}")
        c2 = 0
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
        ...
        self.build_test_data(t3, t4, t5, t6, project)

    def build_test_data(self, t3, t4, t5, t6, project):
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t3 <= self.commit_time[commit] <= t4:
    ...
    def build_link_prediction(self):
        ...
        result_file = open("F:\\task2b_test_imbalance_6time_t1.csv", "w")
