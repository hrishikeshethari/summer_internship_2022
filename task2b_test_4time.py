...
class Task2:
    ...
    def print_data(self, project):
        ...
        print(f"Max time t4 = {t4}")
        y = t1m.year + 3
        t1 = t1m.replace(year=y)
        print(f"t1 = {t1}")
        t2d = (t4 - t1) / 3
        # print(f"t2d = {t2d}")
        t2d_days, t2d_seconds = t2d.days, t2d.seconds
        t2d_hours = t2d_days * 24 + t2d_seconds // 3600
        t2d_minutes = (t2d_seconds % 3600) // 60
        t2d_seconds = t2d_seconds % 60
        # print(f"t2d days = {t2d_days}")
        # print(f"t2d hours = {t2d_hours}, t2d minutes = {t2d_minutes}, t2d seconds = {t2d_seconds}")
        ct = 0
        t2 = 0
        m = 1000
        for commit in self.commit_time:
            ...
            if self.commit_project_data[commit] == project:
                ...
                # temp_minutes = (temp_seconds % 3600) // 60
                # temp_seconds = temp_seconds % 60
                if ct == 1:
                    m = temp_hours - t2d_hours
                if m >= temp_hours - t2d_hours >= 0:
                    t2 = self.commit_time[commit]
                    m = temp_hours - t2d_hours
        print(f"t2 = {t2}")
        t3d = 2 * (t4 - t1) / 3
        # print(f"t3d = {t3d}")
        t3d_days, t3d_seconds = t3d.days, t3d.seconds
        t3d_hours = t3d_days * 24 + t3d_seconds // 3600
        t3d_minutes = (t3d_seconds % 3600) // 60
        t3d_seconds = t3d_seconds % 60
        # print(f"t3d days = {t3d_days}")
        # print(f"t3d hours = {t3d_hours}, t3d minutes = {t3d_minutes}, t3d seconds = {t3d_seconds}")
        ct = 0
        t3 = 0
        m = 1000
        for commit in self.commit_time:
            ...
            if self.commit_project_data[commit] == project:
                ...
                # temp_seconds = temp_seconds % 60
                if ct == 1:
                    m = temp_hours - t3d_hours
                if m >= temp_hours - t3d_hours >= 0:
                    t3 = self.commit_time[commit]
                    m = temp_hours - t3d_hours
        print(f"t3 = {t3}")
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t1 <= self.commit_time[commit] <= t4:
                    c = c + 1
        print(f"Number of commits between t1 and t4 = {c}")
        c2 = 0
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t1 <= self.commit_time[commit] <= t3:
                    c2 = c2 + 1
        print(f"Number of commits between t1 and t3 : {c2}")
        c1 = 0
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t1 <= self.commit_time[commit] <= t2:
                    c1 = c1 + 1
        print(f"Number of commits between t1 and t2 : {c1}")
        c3 = 0
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t2 < self.commit_time[commit] <= t3:
                    c3 = c3 + 1
        print(f"Number of commits between t2 and t3 : {c3}")
        c4 = 0
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t3 < self.commit_time[commit] <= t4:
                    c4 = c4 + 1
        print(f"Number of commits between t3 and t4 : {c4}")
        self.build_test_data(t2, t3, t4, project)

    def build_test_data(self, t2, t3, t4, project):
        for commit in self.commit_time:
            if self.commit_project_data[commit] == project:
                if t2 <= self.commit_time[commit] <= t3:
    ...
    def build_link_prediction(self):
        ...
        result_file = open("F:\\task2b_test_imbalance.csv", "w")
