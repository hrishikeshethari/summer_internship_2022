...
def pair_link_prediction(a, b, z1, z2):
    # a, b, project, g1, g2 = pair[0], pair[1], pair[2], pair[3], pair[4]
    z1.clear()
    z2.clear()
    d = (a, b)
    z1 = set(fake_self.g1.neighbors(a)).intersection(
        set(fake_self.g1.neighbors(b)))
    fake_self.link_common_neighbors[d] = len(z1)
    # breakpoint()
    z2 = set(fake_self.g1.neighbors(a)).union(set(fake_self.g1.neighbors(b)))
    fake_self.link_common_neighbors_union[d] = len(z2)
    fake_self.link_jc[d] = len(z1) / len(z2)
    paa = (nx.adamic_adar_index(fake_self.g1, [d]))
    for pa in paa:
        fake_self.link_aa[d] = pa[2]
    fake_self.link_pa[d] = len(set(fake_self.g1.neighbors(a))) * \
                           len(set(fake_self.g1.neighbors(b)))
    if nx.has_path(fake_self.g1, source=a, target=b):
        fake_self.link_shortest_path[d] = nx.shortest_path_length(
            fake_self.g1, source=a, target=b)
    else:
        fake_self.link_shortest_path[d] = 0
    y1 = fake_self.path_count1(a, b)
    fake_self.link_f_i_f[d] = y1
    y2a = fake_self.path_count2a(a, b)
    fake_self.link_f_a_f[d] = y2a
    y2b = fake_self.path_count2b(a, b)
    fake_self.link_f_c_f[d] = y2b
    y3a = fake_self.path_count3a(a, b)
    fake_self.link_f_l1_f[d] = y3a
    y3b = fake_self.path_count3b(a, b)
    fake_self.link_f_l2_f[d] = y3b
    y3c = fake_self.path_count3c(a, b)
    fake_self.link_f_l3_f[d] = y3c
    y4 = fake_self.path_count4(a, b)
    fake_self.link_f_r_f[d] = y4
    y6 = fake_self.path_count6(a, b)
    fake_self.link_f_i_d_i_f[d] = y6
    y7 = fake_self.path_count7(a, b)
    fake_self.link_f_d_i_f[d] = y7
    y8 = fake_self.path_count8(a, b)
    fake_self.link_f_a_i_f[d] = y8
    y9 = fake_self.path_count9(a, b)
    fake_self.link_f_i_c_i_f[d] = y9
    y11 = fake_self.path_count11(a, b)
    fake_self.link_f_i_a_i_f[d] = y11
    y12 = fake_self.path_count12(a, b)
    fake_self.link_f_i_r_i_f[d] = y12
    if fake_self.g2.has_edge(a, b):
        fake_self.link_class_variable[d] = 1
    else:
        fake_self.link_class_variable[d] = 0
...
class Task2:
    ...
    def __init__(self):
        ...
        globals()["fake_self"] = self
        print(type(fake_self))
    ...
    def build_link_prediction(self, project):
        ...
        c = 0
        for a, b in self.s3:
            # self.pair_link_prediction(a, b, z1, z2)
    ...
    # def pair_link_prediction(self, a, b, z1, z2):
    #     # a, b, project, g1, g2 = pair[0], pair[1], pair[2], pair[3], pair[4]
    #     z1.clear()
    #     z2.clear()
    #     d = (a, b)
    ...
    def execute_multiprocess(self, project):
        # breakpoint()
        for a, b in self.s3:
            Process(target=self.pair_link_prediction,
                    args=([a, b, project, self.g1, self.g2])).start()

        # args = [(ele[0], ele[1], project, self.g1, self.g2) for ele in self.s3]
        ...
        # with Pool() as pool:
        #     breakpoint()
        #     for res in pool.map(self.pair_link_prediction, args):
    ...
    #     with concurrent.futures.ProcessPoolExecutor() as executor:
    #         breakpoint()
    #         results = executor.map(self.pair_link_prediction, args)
    #
    #         for result in results:
    #             print(result)
    ...
    def spawn_link_prediction_pool(self, project):
        ...
        with Pool() as pool:
            lock = Lock()
            results = pool.map(self.pair_link_prediction, (args, lock))
